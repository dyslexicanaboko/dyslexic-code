This is a Microsoft .NET Gadgeteer mainboard project created using the Mainboard Template (2011-08-17).
...
but, following this guide will make your mainboard forward-compatible with forthcoming add-ins.
...
1) In Menu->Project->Properties, change "ManufacturerName" to the short name of your institution with no spaces or punctuation,
   in the two places it appears.  This should match what you print on the mainboard PCB.
...
2) Do a Quick Replace (Ctrl-H) on the Current Project, searching for "ManufacturerName" and replacing with the short name of your 
   ...
   [MainboardName].cs, common.wxi, GadgeteerHardware.xml, and AssemblyInfo.cs.  (NB this does not remove the need for step 1 above.)

3) Edit the [MainboardName].cs file to implement software for your mainboard.
   There are comments and examples in this file to assist you with this process.

4) Testing is most easily accomplished by adding a new Gadgeteer Blank Template project to the same Visual Studio/Visual C# Express solution. 
   With the new Program.cs file open, use the menu item Project->Add Reference, and, in the Projects tab, choose your new mainboard. 
   Then replace InsertMainboardTypeHere() with your mainboard type [Manufacturer Name].Gadgeteer.[Mainboard Name].

5) Edit the GadgeteerHardware.xml file to specify information about your mainboard, as described in that file.

6) Optionally, change Resources\Image.jpg to a good quality top-down image of the mainboard with the socket side facing up,
...
8) Build in Release configuration to build the mainboard installer!
...
If you make want to release a new version of your mainboard, make sure to change the version number in Setup\common.wxi. 
...
As per the Mainboard Builder's Guide, the software mainboard name should match the name printed on the mainboard itself.
The ManufacturerName should match the manufacturer name printed on the mainboard (remove any spaces/punctuation).

==============================================

MAINBOARD TEMPLATE FILE DETAILS

1) [MainboardName].cs - software implementation of the mainboard's "device driver".
2) GadgeteerHardware.xml - defines some parameters about your mainboard.
3) Resources\Image.jpg - placeholder for an image representing the mainboard.
...
6) Setup\msm.wxs - WiX (installer) script that generates an installation merge module for this mainboard.
